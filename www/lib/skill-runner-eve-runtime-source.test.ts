...
describe("packaged EVE runtime source", () => {
  it("installs generated EVE app dependencies without release-age filtering", () => {
    expect(stepsSource).toContain('"$BUN_BIN" install --silent --minimum-release-age=0')
  })

  it("starts generated EVE apps with the EVE start command", () => {
    expect(stepsSource).toContain('"$NODE_RUNTIME" ./node_modules/.bin/eve start')
