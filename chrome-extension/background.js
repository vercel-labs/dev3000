...
class Dev3000Monitor {
  ...
  async attachToTab(tabId) {
    try {
      ...
      this.addLogEntry(`[${new Date().toISOString()}] [TAB-${tabIdentifier}] [ATTACH] Monitoring started - ${tab.title} (${tab.url})`);
      this.addLogEntry(`[${new Date().toISOString()}] [TAB-${tabIdentifier}] [INFO] User-Agent: ${browserInfo.userAgent || 'Unknown'}`);
      this.addLogEntry(`[${new Date().toISOString()}] [TAB-${tabIdentifier}] [INFO] Resolution: ${browserInfo.screenResolution || 'Unknown'}, Window: ${browserInfo.windowSize || 'Unknown'}`);
