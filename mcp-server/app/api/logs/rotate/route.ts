...
import { writeFileSync, existsSync, renameSync, symlinkSync, unlinkSync, readFileSync } from 'fs';
  ...
  try {
    ...
    if (!currentLogPath) {
      return NextResponse.json(
        { error: 'currentLogPath is required' },
        { status: 400 }
      );
    }
    ...
    if (!existsSync(currentLogPath)) {
      return NextResponse.json(
        { error: 'Current log file not found' },
        { status: 404 }
      );
    }
